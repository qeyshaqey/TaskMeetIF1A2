<!doctype html>
<html lang="en">

<head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Dashboard - IF1A2 TASK MEET</title>

    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
    <link href="https://cdn.datatables.net/1.13.6/css/dataTables.bootstrap5.min.css" rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/flatpickr/dist/flatpickr.min.css" rel="stylesheet" />
    <link href="https://cdn.jsdelivr.net/npm/flatpickr/dist/themes/material_blue.css" rel="stylesheet" />

    <style>
        :root {
            --sidebar-w: 250px;
            --blue-1: #784B84;
            --blue-2: #D7BFDC;
        }

        body {
            background: #f4f7fb;
            margin: 0;
            font-family: system-ui, -apple-system, "Segoe UI", Roboto, Arial;
        }

        .navbar {
            background: linear-gradient(90deg, var(--blue-1), var(--blue-2));
            z-index: 1100;
            position: fixed;
            top: 0;
            width: 100%;
        }

        .toggle-floating {
            position: fixed;
            top: 14px;
            left: 14px;
            z-index: 5000;
            background: #ffffff;
            border-radius: 50%;
            width: 46px;
            height: 46px;
            display: flex;
            align-items: center;
            justify-content: center;
            color: var(--blue-1);
            box-shadow: 0 4px 10px rgba(0, 0, 0, 0.1);
        }

        .sidebar {
            width: var(--sidebar-w);
            height: 100vh;
            position: fixed;
            top: 0;
            left: 0;
            padding: 72px 14px 20px 14px;
            background: linear-gradient(180deg, var(--blue-1), var(--blue-2));
            color: #fff;
            transition: left .28s ease;
        }

        .sidebar.hide {
            left: -260px;
        }

        .sidebar .nav-link {
            color: rgba(255, 255, 255, .95);
            border-radius: 8px;
            padding: 10px 12px;
            margin: 6px 6px;
            display: flex;
            align-items: center;
            gap: 10px;
        }

        .sidebar .nav-link:hover,
        .sidebar .nav-link.active {
            background: rgba(255, 255, 255, .14);
        }

        .content {
            margin-left: var(--sidebar-w);
            padding: 90px 24px 36px 24px;
            transition: margin-left .28s ease;
        }

        .sidebar.hide+.content {
            margin-left: 0;
        }

        .filter-bar {
            display: flex;
            gap: 12px;
            align-items: center;
            flex-wrap: wrap;
            margin-bottom: 18px;
        }

        .filter-bar .btn,
        .filter-bar .form-select,
        .filter-bar .form-control {
            height: 40px;
        }

        .card-rail {
            background: #fff;
            border-radius: 10px;
            padding: 18px;
            box-shadow: 0 6px 18px rgba(18, 38, 63, .06);
        }
    </style>
</head>

<body>
    <nav class="navbar navbar-expand-lg navbar-dark fixed-top">
        <div class="container-fluid d-flex align-items-center">
            <button class="toggle-floating" id="btnToggleFloating" title="Toggle menu">
                <i class="fas fa-bars"></i>
            </button>
            <a class="navbar-brand d-flex align-items-center ms-5" href="#">
                <img src="logo.png" alt="logo" style="height:50px; width:auto;">
                <span class="text-white fw-semibold fs-5 ms-2">IF1A2 TASK MEET</span>
            </a>

            <div class="ms-auto d-flex align-items-center">
                <div class="dropdown">
                    <a class="text-white dropdown-toggle d-flex align-items-center" href="#" id="userMenu"
                        data-bs-toggle="dropdown" aria-expanded="false">
                        <i class="fas fa-user-circle fa-lg"></i>
                    </a>
                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="userMenu">
                        <li><a class="dropdown-item" href="profile.html">Profil Saya</a></li>
                        <li>
                            <hr class="dropdown-divider">
                        </li>
                        <li><a class="dropdown-item" href="landing.html">Keluar</a></li>
                    </ul>
                </div>
            </div>

        </div>
    </nav>

    <aside class="sidebar" id="sidebar">
        <h4>Menu</h4>
        <nav class="nav flex-column">
            <a class="nav-link active" href="#"><i class="fas fa-tachometer-alt"></i> Dashboard</a>
            <a class="nav-link" href="Daftar_peserta.html"><i class="fas fa-users"></i> Daftar Peserta</a>
            <a class="nav-link" href="daftar_agenda.html"><i class="fas fa-calendar"></i> Agenda Rapat</a>
        </nav>
    </aside>

    <main class="content">
        <div class="container-fluid">
            <h3 class="mb-3">Selamat Datang</h3>

            <div class="filter-bar">
                <select id="jenisRapat" class="form-select" style="width:200px;">
                    <option value="">Semua Jurusan</option>
                    <option>Teknik Informatika</option>
                    <option>Teknik Mesin</option>
                    <option>Teknik Elektro</option>
                    <option>Manajemen Bisnis</option>
                </select>

                <select id="filterStatus" class="form-select" style="width:200px;">
                    <option value="">Semua Status</option>
                    <option value="akan datang">Akan Datang</option>
                    <option value="berlangsung">Berlangsung</option>
                    <option value="selesai">Selesai</option>
                </select>


                <input id="datePicker" class="form-control" style="width:200px;" placeholder="Pilih tanggal" readonly>
            </div>

            <div class="card-rail">
                <h6 class="mb-3">Jadwal Rapat</h6>
                <div class="table-responsive">
                    <table id="jadwalTable" class="table table-striped table-bordered">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Judul Rapat</th>
                                <th>Jurusan</th>
                                <th>Program Studi</th>
                                <th>Tanggal</th>
                                <th>Waktu</th>
                                <th>Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>1</td>
                                <td>Rapat Koordinasi</td>
                                <td>Teknik Informatika</td>
                                <td>Rekayasa Multimedia</td>
                                <td>2025-04-16</td>
                                <td>08:00</td>
                                <td>SEKARANG</td>
                            </tr>
                            <tr>
                                <td>2</td>
                                <td>Rapat Evaluasi</td>
                                <td>Teknik Informatika</td>
                                <td>Rekayasa Perangkat Lunak</td>
                                <td>2025-04-17</td>
                                <td>08:00</td>
                                <td>AKAN DATANG</td>
                            </tr>
                            <tr>
                                <td>3</td>
                                <td>Rapat Bengkel</td>
                                <td>Teknik Mesin</td>
                                <td>Teknik Otomotif</td>
                                <td>2025-10-22</td>
                                <td>13:00</td>
                                <td>AKAN DATANG</td>
                            </tr>
                            <tr>
                                <td>4</td>
                                <td>Rapat Keuangan</td>
                                <td>Manajemen Bisnis</td>
                                <td>Manajemen Pemasaran</td>
                                <td>2025-09-10</td>
                                <td>09:00</td>
                                <td>SELESAI</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </main>

    <!-- JS -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
    <script src="https://code.jquery.com/jquery-3.7.1.min.js"></script>
    <script src="https://cdn.datatables.net/1.13.6/js/jquery.dataTables.min.js"></script>
    <script src="https://cdn.datatables.net/1.13.6/js/dataTables.bootstrap5.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/flatpickr"></script>

    <script>
        $(document).ready(function() {
            const table = $('#jadwalTable').DataTable({
                pageLength: 5,
                lengthChange: false,
                searching: true,
                ordering: false
            });

            $('#jenisRapat').on('change', function() {
                const jurusan = $(this).val();
                table.column(2).search(jurusan).draw();
            });

            flatpickr("#datePicker", {
                dateFormat: "Y-m-d",
                onChange: function(selectedDates, dateStr) {
                    table.column(4).search(dateStr).draw();
                }
            });

            const sidebar = document.getElementById('sidebar');
            const btnToggle = document.getElementById('btnToggleFloating');
            btnToggle.addEventListener('click', () => {
                if (window.innerWidth < 992) sidebar.classList.toggle('show');
                else sidebar.classList.toggle('hide');
            });
        });

        $(document).ready(function() {

            function loadRapat() {
                $.ajax({
                    url: "http://localhost/TASKMEETIF1A2/admin_php/dahsboard_admin/get_rapat.php",
                    type: "GET",
                    success: function(data) {
                        $('#jadwalTable tbody').empty();
                        data.forEach((item, index) => {
                            $('#jadwalTable tbody').append(`
            <tr>
              <td>${index + 1}</td>
              <td>${item.judul_rapat}</td>
              <td>${item.jurusan}</td>
              <td>${item.program_studi}</td>
              <td>${item.tanggal}</td>
              <td>${item.waktu}</td>
              <td>${item.status.toUpperCase()}</td>
            </tr>
          `);
                        });
                    }
                });
            }

            loadRapat();

            // filter jurusan
            $('#jenisRapat').on('change', function() {
                let jur = $(this).val();
                $.ajax({
                    url: "http://localhost/TASKMEETIF1A2/admin_php/dahsboard_admin/get_rapat.php?jurusan=" + jur,
                    type: "GET",
                    success: function(data) {
                        $('#jadwalTable tbody').empty();
                        data.forEach((item, index) => {
                            $('#jadwalTable tbody').append(`
            <tr>
              <td>${index + 1}</td>
              <td>${item.judul_rapat}</td>
              <td>${item.jurusan}</td>
              <td>${item.program_studi}</td>
              <td>${item.tanggal}</td>
              <td>${item.waktu}</td>
              <td>${item.status.toUpperCase()}</td>
            </tr>
          `);
                        });
                    }
                });
            });

            // filter status
            $('#filterStatus').on('change', function() {
                let st = $(this).val();
                $.ajax({
                    url: "http://localhost/TASKMEETIF1A2/admin_php/dahsboard_admin/get_rapat.php?status=" + st,
                    type: "GET",
                    success: function(data) {
                        $('#jadwalTable tbody').empty();
                        data.forEach((item, index) => {
                            $('#jadwalTable tbody').append(`
                <tr>
                    <td>${index + 1}</td>
                    <td>${item.judul_rapat}</td>
                    <td>${item.jurusan}</td>
                    <td>${item.program_studi}</td>
                    <td>${item.tanggal}</td>
                    <td>${item.waktu}</td>
                    <td>${item.status.toUpperCase()}</td>
                </tr>
                `);
                        });
                    }
                });
            });


        });
    </script>
</body>

</html>